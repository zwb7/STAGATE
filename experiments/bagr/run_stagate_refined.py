"""Retrain vanilla STAGATE on a BAGR-refined spatial graph.

This script is Step 4 of the BAGR protocol. It rebuilds the same input AnnData
used by the vanilla baseline, replaces ``adata.uns['Spatial_Net']`` with a
refined edge list, and then calls the official STAGATE_pyG trainer. The model,
loss, clustering, and evaluation flow are kept aligned with Step 1.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Retrain official STAGATE on a BAGR refined graph."
    )
    parser.add_argument(
        "--baseline-dir",
        type=Path,
        required=True,
        help="Step 1 baseline directory used for spot order and metadata.",
    )
    parser.add_argument(
        "--refined-edges",
        type=Path,
        required=True,
        help="BAGR refined_edges.csv to use as adata.uns['Spatial_Net'].",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=None,
        help="Output directory. Defaults to <refined-edges parent>/refined_stagate.",
    )
    parser.add_argument(
        "--dataset",
        choices=["dlpfc", "h5ad"],
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Input AnnData file when --dataset h5ad.",
    )
    parser.add_argument(
        "--data-root",
        type=Path,
        default=Path("dataset/DLPFC"),
        help="DLPFC dataset root used by --dataset dlpfc.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of mclust clusters. Defaults to baseline metrics n_clusters.",
    )
    ...
    parser.add_argument(
        "--save-h5ad",
        action="store_true",
        help="Save the full AnnData h5ad artifact. Disabled by default.",
    )
    parser.add_argument(
        "--overwrite",
        action="store_true",
...
def resolve_output_dir(args: argparse.Namespace) -> Path:
    ...
    return args.refined_edges.parent / "refined_stagate"
...
def infer_sample_id(args: argparse.Namespace) -> str:
    ...
    metrics = read_json(args.baseline_dir / "metrics_global.json")
    sample_id = metrics.get("sample_id")
    if sample_id:
        return str(sample_id)
    if args.baseline_dir.name.startswith("seed_") and args.baseline_dir.parent.name:
        return args.baseline_dir.parent.name
    return args.baseline_dir.name


def infer_clusters(args: argparse.Namespace) -> int:
    if args.clusters is not None:
        return int(args.clusters)
    metrics = read_json(args.baseline_dir / "metrics_global.json")
    clusters = metrics.get("n_clusters")
    if clusters is None:
        raise ValueError("--clusters is required when baseline metrics do not contain n_clusters")
    return int(clusters)


def load_spot_order(baseline_dir: Path) -> list[str]:
    path = baseline_dir / "pred_labels.csv"
    if not path.exists():
...
def load_dlpfc_adata(args: argparse.Namespace, sample_id: str) -> sc.AnnData:
    ...
    truth_file = args.truth_file or sample_dir / f"{sample_id}_truth.txt"
...
def load_input_adata(args: argparse.Namespace, sample_id: str) -> sc.AnnData:
    if args.dataset == "dlpfc":
        print(f"Loading raw DLPFC slice {sample_id} from {args.data_root}")
        return load_dlpfc_adata(args, sample_id)
...
def align_adata_to_baseline_order(adata: sc.AnnData, spot_order: list[str]) -> sc.AnnData:
    missing = [spot for spot in spot_order if spot not in set(adata.obs_names.astype(str))]
    if missing:
        raise ValueError(
            "Baseline spot order contains spots absent from AnnData, for example: "
            + ", ".join(missing[:5])
        )
    return adata[spot_order, :].copy()


def load_refined_edges(path: Path, obs_names: pd.Index) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Missing refined edge file: {path}")
    edges = pd.read_csv(path, dtype={"Cell1": str, "Cell2": str})
    missing = {"Cell1", "Cell2"}.difference(edges.columns)
    if missing:
        raise ValueError(f"{path} is missing columns: {sorted(missing)}")
    if "Distance" not in edges.columns:
        edges["Distance"] = 1.0
    edges = edges.loc[edges["Cell1"] != edges["Cell2"]].copy().reset_index(drop=True)
    known = set(obs_names.astype(str))
    invalid = ~(edges["Cell1"].isin(known) & edges["Cell2"].isin(known))
    if invalid.any():
        examples = edges.loc[invalid, ["Cell1", "Cell2"]].head().to_dict(orient="records")
        raise ValueError(f"refined_edges.csv contains endpoints absent from AnnData: {examples}")
    return edges
...
def compute_metrics(
    ...
    if ground_truth is None:
        return {"ari": None, "nmi": None, "n_evaluated_spots": 0}
...
def train_and_export(args: argparse.Namespace) -> dict[str, Any]:
    ...
    clusters = infer_clusters(args)
    spot_order = load_spot_order(args.baseline_dir)
    adata = load_input_adata(args, sample_id)
    adata = align_adata_to_baseline_order(adata, spot_order)
    refined_edges = load_refined_edges(args.refined_edges, adata.obs_names)
    adata.uns["Spatial_Net"] = refined_edges
    refined_edges.to_csv(output_dir / "spatial_edges.csv", index=False)
    ...
    print(f"Training refined STAGATE for {sample_id} on {device}")
    ...
    print(f"{sample_id}: Refined Global ARI={ari_text}, NMI={nmi_text}")
    ...
    graph_stats = read_json(args.refined_edges.parent / "graph_refinement_stats.json")
    result: dict[str, Any] = {
        "sample_id": sample_id,
        "method": "bagr_refined_stagate",
        "dataset": args.dataset,
        "baseline_dir": str(args.baseline_dir),
        "refined_edges": str(args.refined_edges),
        ...
        "spatial_edge_count": int(refined_edges.shape[0]),
        "git_commit": get_git_commit(),
        "package_versions": package_versions(),
        "label_mapping_for_correctness": correctness_info["label_mapping"],
        "graph_refinement_stats": graph_stats,
    }
    ...
    if args.save_h5ad:
        adata.write_h5ad(output_dir / f"{sample_id}_bagr_refined_stagate.h5ad")
