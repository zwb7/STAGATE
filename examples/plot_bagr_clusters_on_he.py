"""Plot BAGR clustering labels on the original H&E spatial image.

This script expects an AnnData object with Visium-style spatial metadata
(`adata.uns["spatial"]`) and a CSV/TSV file containing BAGR cluster labels.
It attaches the labels to `adata.obs` and uses `scanpy.pl.spatial`, matching
the visualization style commonly used by STAGATE.

Example
-------
python examples/plot_bagr_clusters_on_he.py \
    --adata results/stagate/151674/adata.h5ad \
    --labels results/bagr/151674/pred_labels.csv \
    --barcode-col barcode \
    --cluster-col bagr_cluster \
    --output results/bagr/151674/bagr_spatial_he.png
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Visualize BAGR clustering results on the original H&E slice."
    )
    parser.add_argument(
        "--adata",
        required=True,
    ...
    parser.add_argument(
        "--cluster-col",
        default="bagr_cluster",
        help="Column in --labels containing BAGR cluster labels.",
    )
    parser.add_argument(
        "--barcode-col",
        default=None,
        help=(
            "Column in --labels containing spot barcodes. If omitted, the first "
            "column is used as barcode."
        ),
    )
    parser.add_argument(
        "--obs-key",
    ...
    parser.add_argument(
        "--sep",
        default=None,
        help="Label-file delimiter. By default pandas infers CSV/TSV delimiter.",
    )
    parser.add_argument(
        "--library-id",
        default=None,
    ...
    parser.add_argument(
        "--title",
        default="BAGR-STAGATE",
        help="Plot title.",
    )
    parser.add_argument(
        "--output",
        required=True,
        help="Output image path, e.g. bagr_spatial_he.png or .pdf.",
    )
    parser.add_argument(
        "--dpi",
        type=int,
    ...
    parser.add_argument(
        "--out-adata",
        default=None,
        help="Optional path to save the AnnData object with attached BAGR labels.",
    )
...
def attach_labels(adata: sc.AnnData, labels: pd.Series, obs_key: str) -> sc.AnnData:
    ...
    if missing_count:
        print(
            f"[WARN] {missing_count} / {adata.n_obs} spots have no BAGR label; "
